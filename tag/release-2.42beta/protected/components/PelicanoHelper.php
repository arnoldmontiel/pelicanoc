...
// 		GATEWAY  =$6
	...
	static public function isPlayerAlive($idPlayer)
	{
		...
		{	
			if(isset($modelPlayer->type)&&$modelPlayer->type==2)
			{
				$isAlive = Mede8erHelper::isPlayerAlive($modelPlayer);
			}
			else if(isset($modelPlayer->type)&&$modelPlayer->type==1)
			{
	...
	public static function updatePendingStateMovies()
	{
		$return = 0;
		$setting = Setting::getInstance();
	...
	public static function updateStateMovies($file_name)
	{
		self::updatePendingStateMovies();
		$return = 0;
		$setting = Setting::getInstance();
