...
class PelicanoHelper
{
	...
	public static function changeFstab()
	{
		...
		try
		{
			...
			exec('echo '.$mjPasswd.'| sudo -u pelicano -S umount '.$file,$output,$return);
			exec('echo '.$mjPasswd.'| sudo -u pelicano -S '.dirname(__FILE__).'/../commands/shell/fstabEditor.sh '.$params,$output,$return);
			exec('echo '.$mjPasswd.'| sudo -u pelicano -S mount -a',$output,$return);
		}
		catch (Exception $e)
		{
		}
